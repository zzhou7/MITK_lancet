...
void DrrGenerator::ItkImageProcessing(const itk::Image<TPixel, VDimension> *itkImage_0)
{
  ...
  // geometry is fully defined. The origin of the CT image becomes irrelevant.
  ...
  transform->SetComputeZYX(true); // rotation order: along X --> along Y --> along Z
  ...
  // The problematice method to retrieve the euler angles
  ...
  // another way to retrieve the euler angles
  ...
  Eigen::Vector3d eulerAngles =
    eigenRotationMatrixWorldToCt.eulerAngles(2, 1, 0); // extrinsic rotation order: along X --> along Y --> along Z in radian
  m_rx = eulerAngles[2];
  m_ry = eulerAngles[1];
  m_rz = eulerAngles[0];
  ...
  int ctPixelNumbers[3]{static_cast<int>(imSize[0]), static_cast<int>(imSize[1]), static_cast<int>(imSize[2])};
  double ctResolution[3]{static_cast<double>(imRes[0]), static_cast<double>(imRes[1]), static_cast<double>(imRes[2])};
  ...
  double translationImagerToInternalCt[3]{m_RaySource[0] - imRes[0] * static_cast<double>(imSize[0]) / 2.0,
  ...
  // mitk::CastToMitkImage(flipFilter->GetOutput(), resultImage); // zzhou: this flipping is actually not required in
