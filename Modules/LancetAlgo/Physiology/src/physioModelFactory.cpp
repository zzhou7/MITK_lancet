...
namespace lancetAlgorithm
{
	...
	void TKA_Model::CalFemurRotation()
	{
		...
		double result = AngleBetween2Vector(FI_BoneResectionLine_Project, PCA_Project, TransverseNormal) * (180.0 / M_PI) * m_resultSymbol;
		SetResult(TKAResult::f_PCAExternal, result);
		result = AngleBetween2Vector(FI_BoneResectionLine_Project, TEA_Project, TransverseNormal) * (180.0 / M_PI) * m_resultSymbol;
	...
	void TKA_Model::CalFemurFlexion()
	{
		...
		double result = AngleBetween2Vector(FIA_Project,MA_Project, SagittalNormal) * (180.0 / M_PI);
	...
	void TKA_Model::CalLimbFlexionAndVarus()
	{
		...
		double result = AngleBetween2Vector(t_MA_Project, f_MA_Project, SagittalNormal) * (180.0 / M_PI);
		SetResult(TKAResult::Limb_Flexion, result);
